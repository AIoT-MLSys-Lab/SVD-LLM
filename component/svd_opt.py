...
class SVDOPTAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.ratio[0] != 1:
            low_rank = int(self.embed_dim * self.ratio[0]/2)
            self.q_u_proj = nn.Linear(low_rank, self.embed_dim, bias=self.enable_bias)
            self.q_v_proj = nn.Linear(self.embed_dim, low_rank, bias=self.enable_bias)
        else:
            self.q_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        if self.ratio[1] != 1:
            low_rank = int(self.embed_dim * self.ratio[1]/2)
            self.k_u_proj = nn.Linear(low_rank, self.embed_dim, bias=self.enable_bias)
            self.k_v_proj = nn.Linear(self.embed_dim, low_rank, bias=self.enable_bias)
        else:
            self.k_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        if self.ratio[2] != 1:
            low_rank = int(self.embed_dim * self.ratio[2]/2)
            self.v_u_proj = nn.Linear(low_rank, self.embed_dim, bias=self.enable_bias)
            self.v_v_proj = nn.Linear(self.embed_dim, low_rank, bias=self.enable_bias)
        else:
            self.v_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        if self.ratio[3] != 1:
            low_rank = int(self.embed_dim * self.ratio[3]/2)
            self.out_u_proj = nn.Linear(low_rank, self.embed_dim, bias=self.enable_bias)
            self.out_v_proj = nn.Linear(self.embed_dim, low_rank, bias=self.enable_bias)
        else:
...
## TODO: Implement OPTAttention2 with SVD
class OptFlashAttention2(OPTAttention):
    ...
    def forward(
        ...
        attn_weights_reshaped = attn_output.reshape(bsz, query_length, self.embed_dim)
...
class SVDOPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, ratio: [1,1]):
        super().__init__()
        ...
        self.ratio = ratio
        if self.ratio[0] != 1:
            low_rank = int(config.ffn_dim * self.embed_dim * self.ratio[0] / (config.ffn_dim + self.embed_dim))
            self.fc1_u_proj = nn.Linear(low_rank, config.ffn_dim, bias=config.enable_bias)
            self.fc1_v_proj = nn.Linear(self.embed_dim, low_rank, bias=config.enable_bias)
        else:
            self.fc1 = nn.Linear(self.embed_dim, config.ffn_dim, bias=config.enable_bias)
        if self.ratio[1] != 1:
            low_rank = int(config.ffn_dim * self.embed_dim * self.ratio[1] / (config.ffn_dim + self.embed_dim))
            self.fc2_u_proj = nn.Linear(low_rank, self.embed_dim, bias=config.enable_bias)
            self.fc2_v_proj = nn.Linear(config.ffn_dim, low_rank, bias=config.enable_bias)
        else:
