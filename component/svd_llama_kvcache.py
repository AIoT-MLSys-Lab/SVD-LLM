...
# TODO: only pass one matrix here rather than q and k since the operation is the same
...
class SVD_LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, compression_ratio=1):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = SVD_LlamaAttention(config=config, compression_ratio=compression_ratio)
        self.mlp = SVD_LlamaMLP(
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([SVD_LlamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
